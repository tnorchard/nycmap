...
export default function Header({
  ...
  const current = BOROUGHS.find((b) => b.id === borough) ?? BOROUGHS[0];
    ...
    <header className="pointer-events-none absolute inset-x-0 top-0 z-[1000] p-3 sm:p-4">
      <div className="pointer-events-auto mx-auto flex max-w-[1400px] flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          <div className="rounded-2xl border border-[#e4e0d8] bg-white px-3.5 py-2.5 shadow-sm">
          ...
          <div className="hidden items-center gap-2 rounded-2xl border border-[#e4e0d8] bg-white px-4 py-2 shadow-sm md:flex">
            ...
            <Divider />
            <Stat label="Loot" value={formatMoney(getTotalRevenue())} />
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={onActivity}
              className="rounded-2xl border border-[#e4e0d8] bg-white px-3 py-2 text-left shadow-sm transition hover:bg-[#f6f4ef]"
            >
              <p className="text-[11px] font-medium text-[#141414]">
                <span className="mr-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-500" />
                {online} casing the joint
              </p>
              <p className="text-[9px] uppercase tracking-[0.14em] text-[#8a847e]">
